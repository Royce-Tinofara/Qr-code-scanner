...
function onScanSuccess(qrCodeMessage) {
  const member = members[qrCodeMessage];
  const qrMessageElement = document.getElementById("qrMessage");
  const memberDetailsElement = document.getElementById("memberDetails");

  if (member && member.subscriptionValid) {
      qrMessageElement.textContent = "Valid member: " + member.name;
      ...
      document.getElementById("exitTime").textContent = "Not exited yet";

      // Add to entry log
      entryLog.push({ id: member.id, name: member.name, entryTime: new Date() });
  } else {
      qrMessageElement.textContent = "Invalid member or subscription";
      memberDetailsElement.style.display = "none";
  }
}
...
document.getElementById("proceedButton").addEventListener("click", function () {
  alert("Proceeding with entry...");
  // Additional logic for entry
});

// Exit button action
document.getElementById("exitButton").addEventListener("click", function () {
  const memberId = document.getElementById("memberId").textContent;
  const member = entryLog.find(entry => entry.id === memberId);
  if (member) {
      member.exitTime = new Date();
      document.getElementById("exitTime").textContent = member.exitTime.toLocaleTimeString();
      alert("Exited at: " + member.exitTime.toLocaleTimeString());
  }
});
var QRCode;
!(function () {
